    ...
    while true; do
        ...
        case $opcao_disco in
            1)
                ...
                printf "%-20s %6s %6s %6s %5s  %s\n" "DISPOSITIVO" "TOTAL" "USADO" "LIVRE" "USO%" "PONTO DE MONTAGEM"
                printf "%-20s %6s %6s %6s %5s  %s\n" "--------------------" "------" "------" "------" "-----" "-----------------"
                # Lista apenas partições reais (/dev/), uma única vez
                df -h | awk 'NR>1 && $1 ~ /^\/dev\// {printf "%-20s %6s %6s %6s %5s  %s\n", $1, $2, $3, $4, $5, $6}'
                pausar
                ;;
            2)
                echo ""
    ...
    while true; do
        ...
        case $opcao_proc in
            1)
                ...
                printf "%-12s %-8s %-6s %-6s %s\n" "------------" "--------" "------" "------" "-------"
                ps aux --sort=-%cpu | awk -v n="$N" 'NR>1 && NR<=n+1 {printf "%-12s %-8s %-6s %-6s %s\n", $1, $2, $3, $4, $11}'
                pausar
                ;;
            2)
                echo ""
